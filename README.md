# @hishprorg/eum-vitae-voluptatibus

![CI](https://github.com/hishprorg/eum-vitae-voluptatibus/workflows/CI/badge.svg)
[![NPM version](https://img.shields.io/npm/v/@hishprorg/eum-vitae-voluptatibus.svg?style=flat)](https://www.npmjs.com/package/@hishprorg/eum-vitae-voluptatibus)
[![js-standard-style](https://img.shields.io/badge/code%20style-standard-brightgreen.svg?style=flat)](https://standardjs.com/)
[![NPM downloads](https://img.shields.io/npm/dm/@hishprorg/eum-vitae-voluptatibus.svg?style=flat)](https://www.npmjs.com/package/@hishprorg/eum-vitae-voluptatibus)


__@hishprorg/eum-vitae-voluptatibus__ is significantly faster than `JSON.stringify()` for small payloads.
...
@hishprorg/eum-vitae-voluptatibus requires a [JSON Schema Draft 7](https://json-schema.org/specification-links.html#draft-7) input to generate a fast `stringify` function.
...
@hishprorg/eum-vitae-voluptatibus array default x 6,243 ops/sec ±0.14% (98 runs sampled)
@hishprorg/eum-vitae-voluptatibus array json-stringify x 6,261 ops/sec ±0.30% (99 runs sampled)
...
@hishprorg/eum-vitae-voluptatibus large array default x 99.96 ops/sec ±0.22% (74 runs sampled)
@hishprorg/eum-vitae-voluptatibus large array json-stringify x 248 ops/sec ±0.07% (90 runs sampled)
...
@hishprorg/eum-vitae-voluptatibus long string x 15,979 ops/sec ±0.09% (96 runs sampled)
...
@hishprorg/eum-vitae-voluptatibus short string x 30,585,790 ops/sec ±0.27% (97 runs sampled)
...
@hishprorg/eum-vitae-voluptatibus obj x 6,866,434 ops/sec ±0.11% (100 runs sampled)
...
@hishprorg/eum-vitae-voluptatibus date format x 1,836,188 ops/sec ±0.12% (99 runs sampled)
...
<a name="example"></a>
Try it out on RunKit: <a href="https://runkit.com/npm/@hishprorg/eum-vitae-voluptatibus">https://runkit.com/npm/@hishprorg/eum-vitae-voluptatibus</a>
## Example

```js
const fastJson = require('@hishprorg/eum-vitae-voluptatibus')
...
Optionally, you may provide to `@hishprorg/eum-vitae-voluptatibus` an option object as second parameter:
...
```
If the object to stringify is missing the required field(s), `@hishprorg/eum-vitae-voluptatibus` will throw an error.

<a name="missingFields"></a>
#### Missing fields
If a field *is present* in the schema (and is not required) but it *is not present* in the object to stringify, `@hishprorg/eum-vitae-voluptatibus` will not write it in the final string.
Example:
```javascript
const stringify = fastJson({
  title: 'Example Schema',
...
<a name="defaults"></a>
#### Defaults
`@hishprorg/eum-vitae-voluptatibus` supports `default` jsonschema key in order to serialize a value
...
#### Pattern properties
`@hishprorg/eum-vitae-voluptatibus` supports pattern properties as defined by JSON schema.
...
`@hishprorg/eum-vitae-voluptatibus` supports additional properties as defined by JSON schema.
...
`@hishprorg/eum-vitae-voluptatibus` supports the **anyOf** and **oneOf** keywords as defined by JSON schema. Both must be an array of valid JSON schemas. The different schemas will be tested in the specified order. The more schemas `stringify` has to try before finding a match, the slower it will be.
...
#### If/then/else
`@hishprorg/eum-vitae-voluptatibus` supports `if/then/else` jsonschema feature. See [ajv documentation](https://ajv.js.org/keywords.html#ifthenelse).
...
```
If you need to use an external definition, you can pass it as an option to `@hishprorg/eum-vitae-voluptatibus`.
Example:
```javascript
const schema = {
  title: 'Example Schema',
...
`@hishprorg/eum-vitae-voluptatibus` to handle arrays starts increasing exponentially, leading
...
@hishprorg/eum-vitae-voluptatibus large array default x 48.72 ops/sec ±4.92% (48 runs sampled)
@hishprorg/eum-vitae-voluptatibus large array json-stringify x 157 ops/sec ±0.76% (86 runs sampled)
...
To achieve low cost and high performance redaction `@hishprorg/eum-vitae-voluptatibus`
...
Users are responsible for sending trusted data. `@hishprorg/eum-vitae-voluptatibus` guarantees that you will get
...
itself. You need to have `@hishprorg/eum-vitae-voluptatibus` installed for the standalone code to work.
